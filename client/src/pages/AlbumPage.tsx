...
import { getAlbum } from "../lib/apiRoutes";
...
const AlbumPage = () => {
  ...
  useEffect(() => {
    ...
    const loadColor = async () => {
      try {
        const color = await extractColorFromImage(album?.logo || "");
        if (isMounted) {
          setCoverColor(color);
        }
      } catch (error) {
      ...
      >
        ...
        <div className="flex px-10 py-8 gap-6">
          ...
          <div className="flex flex-col mt-16 space-y-1">
            <p className="text-md font-semibold text-white">Album</p>
            <h1 className="font-bold text-7xl text-white">{album?.title}</h1>
            <p className="text-sm font-bold text-[#888989] pt-3 flex items-center">
              ...
              · {album?.createdAt?.toLocaleString()} · {album?.songs.length}{" "}
